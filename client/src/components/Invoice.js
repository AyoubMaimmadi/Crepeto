...
let Invoice = ({
  ...
  const names = [
    ...
    },
    {
      ...
      phone: '615223344',
      value: (
        <MenuItem primaryText="yuusuf xusen axmed" secondaryText="615223344" />
      ),
    },
  ]

  const total = salesItems
    .map((salesItem) => salesItem.amount)
    .reduce((a, b) => a + b, 0)
    ...
    <MuiThemeProvider>
      <div>
            ...
            <tbody>
              {salesItems.map((salesItem, index) => (
                <SalesItem
                  ...
                  salesItem={salesItem}
                  items={items}
                  handleEditSalesItem={partial(
                    handleEditSalesItem,
                    salesItem.id
                  )}
                  handleEditSalesItemQty={partial(
                    handleEditSalesItemQty,
                    salesItem.id
                  )}
                  handleEditSalesItemAmount={partial(
                    handleEditSalesItemAmount,
                    salesItem.id
                  )}
                  handleRemoveSalesItem={partial(
                    handleRemoveSalesItem,
                    salesItem.id
                  )}
                />
        ...
        <div style={style.temp}>
          <RaisedButton label="Save" primary={true} style={style.save} />
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    handleEditSalesItem: (id, selectedValue) =>
      dispatch(editSalesItem(id, selectedValue.value)),
    handleEditSalesItemQty: (id, evt) =>
      dispatch(editSalesItemQty(id, evt.target.value)),
    handleEditSalesItemAmount: (id, evt) =>
      dispatch(editSalesItemAmount(id, evt.target.value)),

    handleRemoveSalesItem: (id) => dispatch(removeSalesItem(id)),

    handleEditNewInvoice: (paid) => dispatch(editNewInvoice(paid)),
    handleEditNewInvoiceCustomerName: (customerName) =>
      dispatch(editNewInvoiceCustomerName(customerName)),
    handleEditNewInvoiceCustomerPhone: (evt) =>
      dispatch(editNewInvoiceCustomerPhone(evt.target.value)),
    handleSelectCustomer: (selected) =>
      dispatch(editNewInvoiceSelectCustomer(selected)),
  }
}
